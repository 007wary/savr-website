        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '24px' }}>
          {reasons.map((reason) => (
            <div key={reason.title} className="card" style={{ padding: '32px', position: 'relative', overflow: 'hidden' }}>
              <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '2px', background: 'linear-gradient(90deg, var(--color-primary), transparent)' }} />
              <div style={{ color: 'var(--color-primary)', marginBottom: '20px' }}>{reason.icon}</div>
              <div style={{ display: 'inline-block', fontSize: '11px', fontWeight: '600', color: 'var(--color-primary)', background: 'var(--color-primary-glow)', border: '1px solid var(--color-primary-border)', borderRadius: '100px', padding: '3px 10px', marginBottom: '14px', letterSpacing: '0.5px' }}>
                {reason.tag}
              </div>
              <h3 style={{ fontSize: '20px', fontWeight: '700', marginBottom: '12px' }}>{reason.title}</h3>
              <p style={{ fontSize: '15px', color: 'var(--text-muted)', lineHeight: '1.7' }}>{reason.description}</p>
            </div>
          ))}
        </div>

        {/* Comparison strip */}
        <div style={{ marginTop: '64px', padding: '32px', background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)' }}>
          ...
          {/* Header */}
          <div style={{ display: 'grid', gridTemplateColumns: '40% 20% 20% 20%', borderBottom: '1px solid var(--border)', paddingBottom: '12px' }}>
            {['Feature', 'Savr', 'Wallet', 'Axio'].map((h, i) => (
              <div key={h} style={{ fontSize: '14px', fontWeight: '600', color: i === 1 ? 'var(--color-primary)' : 'var(--text-muted)', textAlign: i === 0 ? 'left' : 'center' }}>
                {h}
              </div>
            ))}
          </div>
          ...
          {/* Rows */}
          {rows.map(([label, ...vals]) => (
            <div key={label} style={{ display: 'grid', gridTemplateColumns: '40% 20% 20% 20%', borderBottom: '1px solid var(--border)', padding: '14px 0', alignItems: 'center' }}>
